...
def test_v9_baseline_registered():
    # 触发 builtin 模块加载即注册
    import importlib
    importlib.import_module("scripts.backtest.strategy.builtin")
...
注意：测试间共享全局注册表。Task 5 的测试用 `_reset_for_test` 清空了，本文件未清空。先跑 builtin 测试再跑 registry 测试可能因顺序导致重复注册。pytest 默认按文件顺序跑，且 registry 测试已通过 fixture 清空——这里不会冲突。如出现 `ValueError: already registered`，说明 builtin 被两次 import：在 `_clean_registry` fixture 后改为：
